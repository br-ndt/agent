...
class Orchestrator:
    ...
    def get_system_prompt(self, session_id: str = "", prompt_tier: str = "full", memory_index: str = "") -> str:
        ...
        result = (
            ...
            "### Executing a skill\n"
            "ONLY execute a skill when the USER explicitly requests it by name or clearly "
            "describes the exact action a skill performs. Do NOT trigger skills based on "
            "keywords that happen to appear in subagent results, conversation context, or "
            "your own reasoning. The user must be the one asking for the action.\n\n"
            "To execute a skill:\n"
    ...
    async def handle(
        ...
        for pass_num in range(self.MAX_PASSES):
            ...
            if valid_delegations:
                ...
                d_results, d_images = await self._execute_delegations(
                    valid_delegations,
                    user_msg,
                    session_id,
                    session=session,
    ...
    async def _execute_delegations(
        ...
        """Run subagent delegations concurrently. Returns (formatted results, generated images).
